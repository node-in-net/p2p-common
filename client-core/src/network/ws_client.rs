        ...
        match cmd {
            NetCmd::Connect(url, new_my_info, turn) => {
                ...
                match tokio::time::timeout(
                    std::time::Duration::from_secs(crate::limits::timeouts().ws_connect_secs),
                    conn_fut,
                )
                .await
                {
            ...
            NetCmd::ApplyTurnCredentials(turn) => {
                self.current_turn_credentials = turn;
                let peers: Vec<String> = self.active_peers.keys().cloned().collect();
                self.handler
                    .on_log(format!(
                        "🔄 [TURN] Relay credentials replaced; redialling {} peer(s)",
                        peers.len()
                    ))
                    .await;
                for id in peers {
                    let _ = self.net_tx_bg.send(NetCmd::DisconnectPeer(id)).await;
                }
            }
            NetCmd::Disconnect => {

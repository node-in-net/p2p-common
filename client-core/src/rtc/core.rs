...
pub async fn handle_core_message(
    ...
    match p2p_msg {
        ...
        P2pMessage::Handshake {
            ...
            if now > timestamp_ms && now - timestamp_ms > 60_000 {
                ...
                handler
                    .on_peer_failed(
                        ...
                        crate::P2pFailure::ClockSkew {
                            ahead_ms: now - timestamp_ms,
                        },
                    )
                    .await;
                handler
            ...
            match found_pub_key {
                Some(pub_key) => {
                    if let Err(e) = nodeinnet_p2p::crypto::verify_p2p_handshake(
                        ...
                        handler
                            .on_log(format!(
                                "❌ Zero-Trust Crypto Verification FAILED for {}: {}",
                                target_node_id, e
                            ))
                            .await;
                        handler
                            .on_peer_failed(
                                target_node_id.clone(),
                                crate::P2pFailure::BadSignature(e.to_string()),
                            )
                            .await;
                        handler
                ...
                None => {
                    ...
                    handler
                        .on_peer_failed(target_node_id.clone(), crate::P2pFailure::UnknownKey)
                        .await;
                    handler
                        .on_peer_state_changed(target_node_id.clone(), crate::P2pPeerState::Failed)
                        .await;
            ...
            handler
                .on_log(format!(
                    "✅ Zero-Trust Cryptographic Connection authenticated with {}",
                    target_node_id
                ))
                .await;
            node_context
            ...
            crate::rtc::spawn_connection_type_poller(
            ...
            if let Some(port) = local_tcp_port {
                ...
                if !ips.is_empty() {
                    handler
                        .on_log(format!(
                            "💾 [CACHE] Caching last known peer IPs for {}: {:?} on port {}",
                            target_node_id, *ips, port
                        ))
                        .await;
                    node_context
                        .peer_store
            ...
            if let Some(c_size) = peer_max_chunk_size_req {
                node_context
                    .peer_max_chunk_size
                    .store(c_size, std::sync::atomic::Ordering::Relaxed);
                handler
                    .on_log(format!(
                        "⚙️ Peer negotiated max chunk size: {} bytes",
                        c_size
                    ))
                    .await;
            }
            ...
            node_context
                .process_message(P2pMessage::Handshake {
                    node_version,
                    timestamp_ms,
                    signature,
                    requested_resources,
        ...
        P2pMessage::Goodbye => {
            handler
                .on_log(format!(
                    "👋 Peer {} sent Goodbye. Closing connection.",
                    target_node_id
                ))
                .await;
        ...
        P2pMessage::Ping(ts) => {
            let envelope = nodeinnet_p2p::SecuredP2pEnvelope {
                mac: None,
                message: P2pMessage::Pong(ts),
            };
            if let Ok(bson_bytes) = nodeinnet_p2p::p2p::to_bson_vec(&envelope) {
                let max_c = 10240;
                let _ = crate::rtc::send_chunked_binary(
                    &dc,
                    &bson_bytes,
                    max_c,
                    &handler,
        ...
        P2pMessage::Pong(ts) => {
            ...
            let rtt = now - ts;
            handler
                .on_log(format!("🏓 P2P Pong received! RTT: {}ms", rtt))
                .await;
            handler
                .on_p2p_ping_updated(target_node_id.clone(), rtt)
                .await;
        }

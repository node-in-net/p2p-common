...
fn ping_interval_secs() -> u64 {
    crate::limits::timeouts().ping_interval_secs
}
fn pong_timeout_ms() -> u64 {
    crate::limits::timeouts().pong_timeout_ms
}
...
fn spawn_rdesk_renegotiation(
    ...
    tokio::spawn(async move {
        ...
        if !wait_until_signaling_stable(
            &pc,
            std::time::Duration::from_secs(crate::limits::timeouts().ice_signaling_stable_secs),
        )
        .await
        {
            let _ = handler
        ...
        match pc.create_offer(None).await {
            Ok(offer) => {
                ...
                let _ = wait_until_signaling_stable(
                    &pc,
                    std::time::Duration::from_secs(
                        crate::limits::timeouts().ice_signaling_stable_secs,
                    ),
                )
                .await;
            }
...
async fn handle_incoming_p2p_message(msg_data: &[u8], ctx: IncomingP2pContext) {
        ...
        nodeinnet_p2p::p2p::from_bson_slice::<nodeinnet_p2p::SecuredP2pEnvelope>(msg_data)
    {
        ...
        } = p2p_msg
        {
            ...
            node_context
                .peer_store
                .update(&mut |map: &mut std::collections::HashMap<
                    String,
                    ...
                    if let Ok(bson_bytes) = nodeinnet_p2p::p2p::to_bson_vec(&p2p_msg) {
                        if !nodeinnet_p2p::crypto::verify_hmac_sha256(
                            ...
                            handler
                                .on_peer_failed(
                                    target_node_id.clone(),
                                    crate::P2pFailure::BadHmac {
                                        resource: res_id.to_string(),
                                    },
                                )
                                .await;
                            handler
            ...
            && !matches!(p2p_msg, P2pMessage::Handshake { .. })
        {
            ...
            if !auth_successful {
                handler
                    ...
                    .await;
                handler
                    .on_peer_failed(target_node_id.clone(), crate::P2pFailure::Unauthenticated)
                    .await;
                handler
                    .on_peer_state_changed(target_node_id.clone(), crate::P2pPeerState::Failed)
                    .await;
        ...
        peer_connection.on_peer_connection_state_change(Box::new(move |s: webrtc::peer_connection::peer_connection_state::RTCPeerConnectionState| {
            ...
            Box::pin(async move {
                ...
                let state = match s {
                    ...
                    webrtc::peer_connection::peer_connection_state::RTCPeerConnectionState::Connected => crate::P2pPeerState::ConnectingTransport,
                ...
                if state == crate::P2pPeerState::Failed {
                    handler
                        .on_peer_failed(target.clone(), crate::P2pFailure::Transport(format!("{s:?}")))
                        .await;
                }
                ...
                if s == webrtc::peer_connection::peer_connection_state::RTCPeerConnectionState::Disconnected {
                    ...
                    tokio::spawn(async move {
                        ...
                        tokio::time::sleep(tokio::time::Duration::from_secs(
                            crate::limits::timeouts().ice_recovery_grace_secs,
                        ))
                        .await;
                        ...
                        tokio::time::sleep(tokio::time::Duration::from_secs(
                            crate::limits::timeouts().ice_teardown_secs,
                        ))
                        .await;
                    ...
                    match payload {
                        nodeinnet_p2p::OutboundP2pPayload::Message(msg) => {
                            if let Ok(bson_bytes) = nodeinnet_p2p::p2p::to_bson_vec(&msg) {
                                ...
                                if crate::limits::is_bulk_stream(&msg.message) {
                                    crate::limits::acquire_send_tokens(bson_bytes.len()).await;
                                }
                        ...
                        nodeinnet_p2p::OutboundP2pPayload::Binary(b) => {
                            ...
                            crate::limits::acquire_send_tokens(b.len()).await;
                            let _ = send_chunked_binary(
                                dc,
                                &b,
                                max_c,
                ...
                Box::pin(async move {
                    ...
                    tokio::spawn(async move {
                        ...
                        loop {
                            ...
                            let ts = std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_millis() as u64;
                            if ts - last_pong_ping.load(Ordering::Relaxed) > pong_timeout_ms() {

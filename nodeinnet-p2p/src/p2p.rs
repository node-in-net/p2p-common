...
pub enum P2pMessage {
    ...
    EntriesResponse {
        ...
        #[serde(default)]
        directories_with_dates: Option<Vec<(String, String)>>,
        #[serde(default)]
        files_with_dates: Option<Vec<(String, u64, String)>>,
        #[serde(default)]
...
impl P2pMessage {
    pub fn allowed_resource_types(&self) -> Vec<ResourceType> {
        match self {
            ...
            | P2pMessage::AppActionResponse { .. } => vec![ResourceType::SharedNetwork],
    ...
    pub fn resource_id(&self) -> Option<&String> {
        match self {
            ...
            P2pMessage::AppListRequest { resource_id, .. } => Some(resource_id),
            P2pMessage::AppListResponse { resource_id, .. } => Some(resource_id),
            P2pMessage::AppLaunchRequest { resource_id, .. } => Some(resource_id),
            P2pMessage::AppStopRequest { resource_id, .. } => Some(resource_id),
            P2pMessage::AppActionResponse { resource_id, .. } => Some(resource_id),
    ...
    pub fn set_resource_id(&mut self, new_id: String) {
        match self {
            ...
            P2pMessage::AppListRequest { resource_id, .. } => *resource_id = new_id,
            P2pMessage::AppListResponse { resource_id, .. } => *resource_id = new_id,
            P2pMessage::AppLaunchRequest { resource_id, .. } => *resource_id = new_id,
            P2pMessage::AppStopRequest { resource_id, .. } => *resource_id = new_id,
            P2pMessage::AppActionResponse { resource_id, .. } => *resource_id = new_id,
...
#[cfg(test)]
mod remote_launch_registration {
    use super::*;

    fn every_variant() -> Vec<P2pMessage> {
        let id = Uuid::new_v4();
        vec![
            P2pMessage::AppListRequest {
                resource_id: "placeholder".into(),
                request_id: id,
            },
            P2pMessage::AppListResponse {
                resource_id: "placeholder".into(),
    ...
    #[test]
    fn every_variant_is_anchored_to_the_network_resource() {
        for msg in every_variant() {
            let allowed = msg.allowed_resource_types();
            assert_eq!(
                allowed,
                vec![ResourceType::SharedNetwork],
                "{msg:?} is not anchored — an empty list disables the type check entirely"
            );
        }
    }

    #[test]
    fn every_variant_names_its_resource() {
        for msg in every_variant() {
            assert!(
                msg.resource_id().is_some(),
                "{msg:?} has no resource id, so it would travel unsigned and ungated"
            );
        }
    }

    #[test]
    fn every_variant_can_be_retagged() {
        for mut msg in every_variant() {
            msg.set_resource_id("network-abc".to_string());
            assert_eq!(
                msg.resource_id().map(String::as_str),
                Some("network-abc"),
                "{msg:?} kept its old id — the arm in set_resource_id is missing"
            );
        }
    }
    ...
    fn the_wire_shape_survives_a_round_trip() {
        for msg in every_variant() {
            let bytes = to_bson_vec(&msg).expect("serialises");
            let back: P2pMessage = from_bson_slice(&bytes).expect("deserialises");
            assert_eq!(
                format!("{back:?}"),
                format!("{msg:?}"),

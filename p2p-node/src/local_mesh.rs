...
async fn handle_incoming_tcp_tunnel(
    ...
    {
        let pub_key = nodeinnet_p2p::get_known_public_key(initiator_id)
            .ok_or_else(|| format!("No known public key for initiator {}", initiator_id))?;
        ...
        .map_err(|e| format!("handshake signature verification failed: {}", e))?;
        ...
        P2pMessage::ResponseTopology { peers } => {
            for peer in peers {
                ...
                if nodeinnet_p2p::get_known_public_key(&peer.id).is_none() {
                    ...
                    peer_store.update(&mut |map: &mut std::collections::HashMap<
                        String,
